...
- threads_post와 별개로, 오늘 내용을 3~4개의 짧은 게시물로 나눠 서로 답글로
  이어지는 "진짜 스레드" 형태도 함께 만드세요. 각 항목은 100~150자.
  순서: ① 훅+관점(threads_post와 유사한 역할) → ② 그 관점을 뒷받침하는
  데이터/근거 1개 → ③ 반대 시나리오나 리스크 요인 1개(선택) → ④ 한 줄 정리
  + 답글 유도 질문. 해시태그는 마지막 항목에만 1~3개 붙이세요.
- 각 항목은 그 자체로 완결된 문장이어야 합니다 ("그리고", "또한"으로 앞
...
반드시 아래 JSON 형식으로만 응답하세요 (마크다운 코드블록 없이 순수 JSON만):
{
  ...
  "threads_thread": [
    "① 훅+관점 (100~150자)",
    "② 근거/데이터 (100~150자)",
    "③ 리스크·반대 시나리오 (선택, 100~150자)",
    "④ 정리 + 답글 유도 질문 + 해시태그 1~3개 (100~150자)"
  ],
