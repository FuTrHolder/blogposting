...
class TistoryUploader:
    ...
    def upload(
        ...
        with sync_playwright() as p:
            ...
            try:
                # 1) 티스토리 경유 카카오 로그인 (세션 확보)
                self._login_via_tistory(page)
    ...
    def _login_via_tistory(self, page):
        """
        티스토리 OAuth 진입 → 리다이렉트된 카카오 로그인 페이지에서 로그인.
        카카오 로그인은 accounts.kakao.com에 직접 접근하면 400 에러가 나므로
        반드시 티스토리를 통해 진입해야 합니다.
        """
        logger.info("티스토리 OAuth 진입 중...")
        page.goto(TISTORY_AUTH_URL, wait_until="domcontentloaded", timeout=30000)
        human_delay(2, 3)

        current_url = page.url
        logger.info(f"OAuth 후 URL: {current_url}")

        # 카카오 로그인 페이지로 리다이렉트된 경우
        if "accounts.kakao.com" in current_url:
            logger.info("카카오 로그인 페이지 진입 확인. 로그인 시작...")
            self._fill_kakao_form(page)

        # 이미 티스토리 세션이 있는 경우 (로그인 페이지 안 뜸)
        elif "tistory.com" in current_url:
            logger.info(f"티스토리 세션 확인: {current_url}")

        else:
            logger.warning(f"예상치 못한 URL: {current_url}")
    ...
    def _fill_kakao_form(self, page):
        """카카오 로그인 폼에 이메일/비밀번호를 입력합니다."""
        # 이메일 입력창 대기 (티스토리가 리다이렉트한 카카오 페이지)
        ...
        human_delay(2, 3)
        logger.info(f"카카오 로그인 완료. URL: {page.url}")
